...
export interface ICategory extends Document {
  ...
  level: number;
  path: string; // 分类路径，如 "/electronics/computers/laptops"
...
CategorySchema.pre('save', async function(next) {
  if (this.isModified('parent_category') || this.isModified('slug')) {
    if (this.parent_category) {
      const parent = await (this.constructor as any).findById(this.parent_category);
      if (parent) {
...
CategorySchema.pre('deleteOne', async function(next) {
  ...
  if (doc && doc.parent_category) {
    await (this.model as any).findByIdAndUpdate(
      doc.parent_category,
...
CategorySchema.statics.getCategoryTree = async function(parentId = null, maxDepth = 3) {
  ...
  if (maxDepth > 0) {
    for (const category of categories) {
      (category as any).children = await (this as any).getCategoryTree(category._id, maxDepth - 1);
    }
  }
...
// 根据路径查找分类
CategorySchema.statics.findByPath = function(path: string) {
...
CategorySchema.methods.getAncestors = async function() {
  ...
  while (current.parent_category) {
    current = await (this.constructor as any).findById(current.parent_category);
    if (current) {
...
CategorySchema.methods.getDescendants = async function() {
  ...
  const getChildren = async (categoryId: any) => {
    const children = await (this.constructor as any).find({
...
// 移动分类
CategorySchema.methods.moveTo = async function(newParentId: any) {
  // 移除旧父分类的引用
  if (this.parent_category) {
    await (this.constructor as any).findByIdAndUpdate(
  ...
  // 重新计算层级和路径
  if (newParentId) {
    const newParent = await (this.constructor as any).findById(newParentId);
    if (newParent) {
  ...
  for (const descendant of descendants) {
    ...
    descendant.path = '/' + [...ancestors.map((a: any) => a.slug), descendant.slug].join('/');
