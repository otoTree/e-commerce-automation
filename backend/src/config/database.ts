...
export const dbUtils = {
  // 清理过期数据
  cleanupExpiredData: async (collectionName: string, expiryField: string, expiryTime: Date): Promise<number> => {
    try {
      const db = mongoose.connection.db;
      if (!db) {
  ...
  // 获取集合统计信息
  getCollectionStats: async (collectionName: string): Promise<any> => {
    try {
      const db = mongoose.connection.db;
      if (!db) {
        throw new Error('数据库连接不可用');
  ...
  // 创建索引
  createIndex: async (collectionName: string, indexSpec: any, options?: any): Promise<string> => {
    try {
      const db = mongoose.connection.db;
      if (!db) {
        throw new Error('数据库连接不可用');
