...
import type { TaskCreateRequest, TaskCompleteRequest } from '../types/index.js';
...
router.get('/ai-selected-products', async (req: Request, res: Response) => {
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const newTask = await taskService.createDatabaseTask({
      ...
      type: mappedType as 'data_collection' | 'content_generation' | 'analysis' | 'optimization' | 'custom',
      config: taskData.config || taskData.content || {},
...
// 接收爬取结果
router.post('/:taskId/complete', (req: Request<{ taskId: string }, {}, TaskCompleteRequest>, res: Response) => {
...
// 获取AI建议
router.post('/:id/ai-suggestions', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
