 ...
 * 清理过期任务
 */
const cleanupExpiredTasks = async (): Promise<number> => {
  try {
 ...
 * 清理过期分析结果
 */
const cleanupExpiredAnalyses = async (): Promise<number> => {
  try {
 ...
 * 清理临时文件和缓存
 */
const cleanupTempData = async (): Promise<number> => {
  try {
    // 这里可以添加清理临时文件、缓存等逻辑
    // 目前返回0，表示没有清理任何临时数据
    return 0;
 ...
 * 执行完整的清理任务
 */
const performCleanup = async (): Promise<CleanupStats> => {
 ...
 * 停止清理任务
 */
export const stopCleanupTask = (): void => {
  if (cleanupInterval) {
 ...
 * 手动执行清理任务
 */
export const runCleanupNow = async (): Promise<CleanupStats> => {
 ...
 * 更新清理配置
 */
export const updateCleanupConfig = (newConfig: Partial<typeof CLEANUP_CONFIG>): void => {
