...
// 1688商品解析器
export const alibaba1688ProductParser: IProductParser = {
  platform: 'alibaba',
  ...
  parse: async (html: string, url: string): Promise<ParseResult<Partial<IProductFullData>>> => {
    ...
    try {
      ...
      const productData: Partial<IProductFullData> = {
        platform: 'alibaba',
...
// Ozon商品解析器
export const ozonProductParser: IProductParser = {
  platform: 'ozon',
...
// 解析器注册表
export const productParsers: IProductParser[] = [
  alibaba1688ProductParser,
  ozonProductParser
];

// 获取适合的解析器 - 改进的自动识别逻辑
export const getProductParser = (url: string, html?: string): IProductParser | null => {
  // 首先基于URL进行识别
...
// 智能平台识别函数
export const identifyPlatform = (url: string, html?: string): string => {
  // URL优先识别
