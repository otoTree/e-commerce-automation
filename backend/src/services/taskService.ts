...
class TaskService {
  private crawlTasks = new Map<string, CrawlTask>();
  ...
  getAvailableTasks(limit: number = 5): CrawlTask[] {
    return Array.from(this.crawlTasks.values())
  ...
  completeTask(taskId: string, data?: any, success: boolean = true, error?: string): boolean {
  ...
  // 更新数据库任务状态
  async updateDatabaseTaskStatus(taskId: string, success: boolean, data?: any, error?: string): Promise<void> {
    try {
  ...
  // 保存商品数据到数据库
  private async saveProductData(taskId: string, data: any): Promise<void> {
    try {
