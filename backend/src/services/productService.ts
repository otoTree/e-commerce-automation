import { Product, type IProduct, type I1688ProductData } from '../models/Product.js';
...
export class ProductService {
   ...
   * 更新商品
   */
  static async updateProduct(id: string, updateData: UpdateProductData): Promise<IProduct | null> {
    try {
      if (!Types.ObjectId.isValid(id)) {
        throw new Error('无效的商品ID');
      }
   ...
   * 添加商品评分
   */
  static async addRating(id: string, rating: number): Promise<IProduct | null> {
    try {
      if (!Types.ObjectId.isValid(id)) {
        throw new Error('无效的商品ID');
      }
   ...
   * 根据品牌获取商品
   */
  static async getProductsByBrand(brand: string): Promise<IProduct[]> {
    try {
   ...
   * 根据价格范围获取商品
   */
  static async getProductsByPriceRange(minPrice: number, maxPrice: number): Promise<IProduct[]> {
    try {
   ...
   * 获取热门商品（根据评分排序）
   */
  static async getPopularProducts(limit: number = 10): Promise<IProduct[]> {
    try {
   ...
   * 获取最新商品
   */
  static async getLatestProducts(limit: number = 10): Promise<IProduct[]> {
    try {
      return await Product.find({ status: 'active' })
        .sort({ createdAt: -1 })
        .limit(limit)
   ...
   * 批量导入商品
   */
  static async bulkCreateProducts(productsData: CreateProductData[]): Promise<IProduct[]> {
    try {
   ...
   * 清空所有商品
   */
  static async clearAllProducts(): Promise<{ deletedCount: number }> {
    try {
...
// 1688商品数据转换函数
export const convert1688ToProduct = (data: I1688ProductData): CreateProductData => {
  // 生成SKU（使用链接的最后部分）
...
// 批量处理1688商品数据
export const process1688Products = async (products: I1688ProductData[]) => {
  const results = {
    success: 0,
    failed: 0,
    errors: [] as string[]
  ...
  for (const productData of products) {
    try {
      ...
      // 检查是否已存在相同SKU的商品
       ...
       if (existingProduct) {
         // 更新现有商品
         await ProductService.updateProduct((existingProduct._id as Types.ObjectId).toString(), {
