...
class TaskExecutor {
  ...
  private pollInterval = 5000; // 5秒检查一次
  ...
  private async executeDataCollection(task: any) {
    const urls = task.input.product_urls || [];
    const collectedProducts: string[] = [];
    let successCount = 0;
    let failureCount = 0;
  ...
  private async executeKeywordCollection(task: any) {
    // 关键词收集逻辑
  ...
  private async executeDeepAnalysis(task: any) {
  ...
  private async performDeepAnalysis(productId: string, options: any): Promise<string | null> {
    try {
  ...
  // 计算竞争力评分
  private calculateCompetitivenessScore(product: any): number {
    let score = 0;
  ...
  // 分析价格优势
  private analyzePriceAdvantage(product: any): number {
  ...
  // 分析质量指标
  private analyzeQualityIndicators(product: any): number {
    let score = 0;
  ...
  // 计算供应商可靠性
  private calculateSupplierReliability(product: any): number {
  ...
  // 分析产品独特性
  private analyzeProductUniqueness(product: any): number {
    let score = 50; // 基础分
  ...
  // 生成竞争力洞察
  private generateCompetitivenessInsights(product: any): string[] {
  ...
  // 计算盈利评分
  private calculateProfitScore(product: any): number {
  ...
  // 计算估算利润率
  private calculateEstimatedMargin(product: any): number {
    const price = product.pricing.current_price;
    if (price === 0 || price === null || price === undefined) {
  ...
  // 估算运费
  private estimateShippingCost(product: any): number {
    const price = product.pricing.current_price;
    if (price === 0 || price === null || price === undefined) {
      return 10; // 默认运费
    }
  ...
  // 分析目标细分市场
  private analyzeTargetSegment(product: any): string {
    const price = product.pricing.current_price;
    const category = product.basic_info.category || '通用';
    
    if (price < 50) return `${category}入门级市场`;
    if (price < 200) return `${category}中端市场`;
    return `${category}高端市场`;
  }

  // 分析价格层级
  private analyzePriceTier(product: any): 'low' | 'mid' | 'high' {
    const price = product.pricing.current_price;
  ...
  // 分析差异化要点
  private analyzeDifferentiationPoints(product: any): string[] {
    const points: string[] = [];
  ...
  // 分析竞争格局
  private analyzeCompetitiveLandscape(product: any): string {
  ...
  // 计算热度评分
  private calculateHeatScore(product: any): number {
    let score = 0;
    
    // 基于销量
    const salesVolume = product.sales_data.sales_volume;
    if (salesVolume > 1000) score += 40;
    else if (salesVolume > 100) score += 25;
    else if (salesVolume > 10) score += 15;
  ...
  // 分析热度趋势
  private analyzeHeatTrend(product: any): 'rising' | 'stable' | 'declining' {
    const salesVolume = product.sales_data.sales_volume;
    const rating = product.sales_data.rating;
    
    if (salesVolume > 500 && rating > 4.0) return 'rising';
    if (salesVolume > 50 && rating > 3.5) return 'stable';
    return 'declining';
  }

  // 分析搜索量
  private analyzeSearchVolume(product: any): number {
    const salesVolume = product.sales_data.sales_volume;
    return Math.min(100, salesVolume / 10);
  }

  // 分析销售速度
  private analyzeSalesVelocity(product: any): number {
    const salesVolume = product.sales_data.sales_volume;
    if (salesVolume > 1000) return 90;
    if (salesVolume > 100) return 70;
    if (salesVolume > 10) return 50;
    return 20;
  }

  // 分析价格稳定性
  private analyzePriceStability(product: any): number {
    // 简化实现，假设价格相对稳定
    return product.pricing.current_price > 0 ? 75 : 30;
  }

  // 分析季节性因素
  private analyzeSeasonalFactor(product: any): number {
    // 简化实现，返回中性值
    return 50;
  }

  // 计算总评分
  private calculateTotalScore(product: any): number {
  ...
  // 生成推荐
  private generateRecommendation(product: any): 'strong_buy' | 'buy' | 'hold' | 'avoid' {
  ...
  // 生成关键原因
  private generateKeyReasons(product: any): string[] {
  ...
  // 识别风险因素
  private identifyRiskFactors(product: any): string[] {
    const risks: string[] = [];
  ...
  // 分析图片质量
  private analyzeImageQuality(images: string[]): number {
    if (!images || images.length === 0) return 0;
    if (images.length >= 5) return 90;
  ...
  private analyzeDescriptionQuality(description: string): number {
    ...
    if (description.length > 200) return 90;
    if (description.length > 100) return 70;
    if (description.length > 50) return 50;
    return 30;
  }

  // 标记任务失败
  private async markTaskFailed(task: any, error: any) {
    task.status = 'failed';
    task.meta.completed_at = new Date();
  ...
  private getSiteConfig(url: string) {
  ...
  private async extractProductData(url: string): Promise<ProductData | null> {
    try {
      ...
      const axiosConfig: any = {
        ...
        validateStatus: (status: number) => status < 400 // 接受所有小于400的状态码
      };
  ...
  // 备用解析方法 - 当解析器不可用时使用
  private fallbackExtractProductData(url: string, html: string): ProductData | null {
    try {
  ...
  // 保存商品数据
  private async saveProductData(productData: ProductData): Promise<string> {
    try {
  ...
  // 辅助方法：从销量字符串中提取数字
  private extractSalesNumber(salesStr: string): number {
  ...
  // 辅助方法：计算数据完整度
  private calculateDataCompleteness(productData: ProductData): number {
    let completeness = 0;
    const fields = ['title', 'price', 'supplier', 'sales'];
    const imageWeight = 0.2;
    
    fields.forEach(field => {
      if (productData[field as keyof ProductData] && 
          productData[field as keyof ProductData] !== '未知商品' && 
          productData[field as keyof ProductData] !== '价格面议' && 
          productData[field as keyof ProductData] !== '未知供应商' && 
          productData[field as keyof ProductData] !== '0') {
        completeness += 0.2;
      }
    });
