...
// 获取待执行任务
router.post('/tasks/poll', async (req, res) => {
    try {
        const { extension_id } = req.body;
        if (!extension_id) {
            return res.status(400).json({
...
// 提交任务结果接口
router.post('/tasks/result', async (req, res) => {
    try {
    ...
    for (const productData of products) {
        try {
            ...
            if (existingProduct) {
                ...
                savedProductIds.push(existingProduct._id.toString());
            }
            else {
                // 创建新商品
                ...
                savedProductIds.push(newProduct._id.toString());
            }
        }
        catch (error) {
...
// 后端调用插件获取源码
router.post('/request-source', async (req, res) => {
    try {
...
// 获取源码任务结果
router.get('/source-task/:taskId', async (req, res) => {
    try {
