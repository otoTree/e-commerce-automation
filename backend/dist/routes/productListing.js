 ...
 */
const getAnalyzedProducts = async (req, res) => {
    try {
        const { platform = 'other', category, min_score = 0, has_analysis = true, page = 1, limit = 20, sort = '-analysis_meta.analyzed_at' } = req.query;
        // 获取商品列表
...
const getListings = async (req, res) => {
    try {
        const { status, platform = 'ozon', created_by, page = 1, limit = 20, sort = '-meta.created_at' } = req.query;
        // 构建查询条件
...
const updateListing = async (req, res) => {
    try {
        ...
        const listing = await ProductListing.findByIdAndUpdate(listingId, {
            $set: {
                ...updateData,
 ...
 */
const batchOperations = async (req, res) => {
    try {
        ...
        const result = await ProductListing.updateMany({ _id: { $in: listing_ids } }, {
            $set: {
                ...updateData,
                'meta.updated_at': new Date()
            }
