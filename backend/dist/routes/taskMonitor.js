...
const updateTask = async (req, res) => {
    try {
        ...
        const task = await TaskModel.findOneAndUpdate({ task_id: taskId }, updateData, { new: true });
        if (!task) {
            return res.status(404).json({
                success: false,
                error: '任务不存在'
...
const batchUpdateTasks = async (req, res) => {
    try {
        ...
        const result = await TaskModel.updateMany({ task_id: { $in: taskIds } }, updateData);
        res.json({
            success: true,
            data: {
 ...
 */
const exportTaskResults = async (req, res) => {
    try {
        const { taskId } = req.params;
...
const getTaskStats = async (req, res) => {
    try {
        const [totalTasks, runningTasks, completedTasks, failedTasks, pendingTasks, pausedTasks] = await Promise.all([
