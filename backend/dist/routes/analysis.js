 ...
 */
const getMarketHeatData = async (req, res) => {
    try {
        const { productId } = req.params;
        const analysisResult = await DeepAnalysisResult.findOne({ product_id: productId })
 ...
 */
const getBatchAnalysisResults = async (req, res) => {
    try {
...
const getAnalysisResults = async (req, res) => {
    try {
        ...
        const results = analysisResults.map(result => ({
            ...
            productTitle: result.product_id?.title || '未知商品',
            marketHeat: {
 ...
 */
const getAnalysisTasks = async (req, res) => {
    try {
        const { status, page = 1, limit = 20, sort = '-meta.created_at' } = req.query;
        const filter = { type: 'deep_analysis' };
        if (status) {
...
const getAnalysisStatistics = async (req, res) => {
    try {
        const [totalAnalyses, recentAnalyses, recommendationStats, heatTrendStats] = await Promise.all([
            // 总分析数量
