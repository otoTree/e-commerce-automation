 ...
 */
const batchCollectProducts = async (req, res) => {
    try {
 ...
 */
const collectByKeywords = async (req, res) => {
    try {
 ...
 */
const getCollectionTasks = async (req, res) => {
    try {
        const { status, type, page = 1, limit = 20, sort = '-meta.created_at' } = req.query;
        const filter = {};
        // 支持按任务类型过滤
        if (type) {
            filter.type = type;
 ...
 */
const submitHtml = async (req, res) => {
    try {
        // 详细的请求调试日志
        ...
        res.json({
            success: true,
            task_id: htmlStorage._id.toString()
        });
    }
    catch (error) {
            ...
            if (htmlStorage.page_type === 'product') {
                ...
                if (parseResult.success && parseResult.data) {
                    ...
                    return res.json({
                        ...
                        message: '搜索页解析完成',
                        html_storage_id,
                        search_data_id: searchData._id.toString(),
 ...
 */
const batchParseHtml = async (req, res) => {
    try {
        const { platform, page_type, limit = 10, force_reparse = false } = req.body;
        // 构建查询条件
        const query = {};
        ...
        for (const htmlRecord of htmlRecords) {
            try {
                ...
                results.push({
                    html_storage_id: htmlRecord._id.toString(),
...
// 辅助函数：解析单个HTML记录
const parseHtmlRecord = async (htmlStorage, forceReparse = false) => {
 ...
 */
const processSearchData = async (req, res) => {
    try {
        const { platform, limit = 10, auto_trigger_product_collection = true } = req.body;
        // 构建查询条件：查找未处理的搜索页数据
        const query = {
            is_processed: false,
...
// 辅助函数：处理单个搜索页数据记录
const processSearchDataRecord = async (searchData, autoTriggerCollection = true) => {
    try {
