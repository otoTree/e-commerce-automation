...
// 1688搜索页解析器
export const alibaba1688SearchParser = {
    platform: 'alibaba',
    canParse: (url) => {
        ...
        try {
            ...
            $('.offer-item, .product-item, .sm-offer-item').each((_, element) => {
                ...
                if (title && productUrl) {
                    ...
                    if (imageUrl)
                        productItem.image_url = normalizeUrl(imageUrl);
                    if (supplierName)
                        productItem.supplier_name = supplierName;
                    if (supplierUrl)
                        productItem.supplier_url = normalizeUrl(supplierUrl);
                    if (salesInfo)
                        productItem.sales_info = salesInfo;
                    if (location)
            ...
            const searchPageData = {
                ...
                pagination_info: (() => {
                    ...
                    if (totalPages !== undefined)
                        paginationInfo.total_pages = totalPages;
                    if (nextPageUrl)
                        paginationInfo.next_page_url = normalizeUrl(nextPageUrl);
            ...
            $('[data-widget="searchResultsV2"] .tile, .product-card').each((_, element) => {
                ...
                if (title && productUrl) {
                    ...
                    if (reviewCount !== undefined)
                        productItem.review_count = reviewCount;
                    products.push(productItem);
                }
            });
            // 提取分页信息
...
// 工具函数
const extractSearchKeyword = (url) => {
...
// 获取适合的搜索页解析器
export const getSearchPageParser = (url) => {
    return searchPageParsers.find(parser => parser.canParse(url)) || null;
};
//# sourceMappingURL=searchPageParser.js.map