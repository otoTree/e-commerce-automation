...
CartSchema.virtual('has_stock_issues').get(function () {
    return this.items.some((item) => item.stock_status === 'out_of_stock' ||
        item.quantity > item.available_quantity);
});
// 是否有不可用的商品
CartSchema.virtual('has_unavailable_items').get(function () {
    return this.items.some((item) => !item.is_available);
});
// 购物车年龄（天）
...
// 静态方法
// 根据用户查找购物车
CartSchema.statics.findByUser = function (userId, cartType = 'regular') {
    return this.findOne({
        user_id: userId,
        cart_type: cartType,
        status: 'active',
...
// 根据会话查找购物车
CartSchema.statics.findBySession = function (sessionId, cartType = 'regular') {
    return this.findOne({
...
CartSchema.methods.addItem = function (itemData) {
    const existingItemIndex = this.items.findIndex((item) => item.product_id.toString() === itemData.product_id.toString() &&
...
CartSchema.methods.applyCoupon = function (couponData) {
    // 检查优惠券是否已应用
    const existingCoupon = this.coupons.find((coupon) => coupon.code === couponData.code);
    if (existingCoupon) {
...
// 添加历史记录
CartSchema.methods.addHistory = function (action, details) {
    this.history.push({
        timestamp: new Date(),
        action,
        details,
...
// 合并购物车
CartSchema.methods.mergeWith = function (otherCart, strategy = 'combine_quantities') {
    const mergedItems = [...this.items];
    otherCart.items.forEach((otherItem) => {
        const existingItemIndex = mergedItems.findIndex((item) => item.product_id.toString() === otherItem.product_id.toString() &&
