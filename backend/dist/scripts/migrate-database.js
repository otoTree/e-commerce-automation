...
class DatabaseMigrator {
        ...
        this.migrations = [
            ...
            },
            {
                ...
                up: async () => {
                    ...
                    await users.updateMany({ preferences: { $exists: false } }, {
                        $set: {
                            preferences: {
                                language: 'zh-CN',
                                currency: 'CNY',
            ...
            },
            {
                ...
                up: async () => {
                    ...
                    await products.updateMany({ seo: { $exists: false } }, {
                        $set: {
                            seo: {
                                meta_title: '',
                                meta_description: '',
                ...
                down: async () => {
                    ...
                    const products = this.db.collection('products');
                    await products.updateMany({}, { $unset: { seo: '' } });
                }
            },
            {
                version: '1.3.0',
                ...
                up: async () => {
                    ...
                    await orders.updateMany({ 'shipping.tracking': { $exists: false } }, {
                        $set: {
                            'shipping.tracking': {
                                tracking_number: '',
                                carrier: '',
                ...
                down: async () => {
                    ...
                    const orders = this.db.collection('orders');
                    await orders.updateMany({}, { $unset: { 'shipping.tracking': '' } });
                }
            },
            {
                version: '1.4.0',
    ...
    async getAppliedMigrations() {
        try {
            ...
            return await migrations.find({}).sort({ appliedAt: 1 }).toArray();
        }
        catch (error) {
    ...
    async rollback(targetVersion) {
        ...
        if (targetVersion) {
            migrationsToRollback = appliedMigrations.filter(m => this.compareVersions(m.version, targetVersion) > 0);
        }
        else {
            // 如果没有指定目标版本，只回滚最后一个
